...
某些存储库（包括 .NET 存储库）没有启用“PR 合并”web 挂钩，这会自动合并次要 PR。 本文介绍这些存储库的 PR 评审流程。 PR 评审流程的设计目标是：

- 发布来自团队、产品团队成员和社区成员的高质量内容。
- 以一致的方式向作者提供及时、可操作的反馈。
- 促进作者和审阅者之间的讨论。
...
- 正在进行的工作 - 作者可以通过打开草稿拉取请求来创建标记为“尚未准备好进行评审”的 PR。
...
作者更新 PR 以响应评论，用“+1”回应，标记每个已处理的评论，表明它已得到处理。 如果作者不同意评论内容，或者在其他 PR 中处理评论，他会添加评论来解释这些变更。
